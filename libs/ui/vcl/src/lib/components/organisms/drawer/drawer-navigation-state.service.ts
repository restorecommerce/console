...
import { map, filter, take, audit, share } from 'rxjs/operators';
...
export type NavigationAction =
  | NavigationEnterItemAction
  | NavigationLeaveItemAction
  | NavigationSelectItemAction
  | NavigationEnterNavAction
...
export class NavigationStateService {
  ...
  state$: Observable<NavigationState> = this.actionEmitter.pipe(
    audit((action) => {
      ...
      if (
        action &&
        action.type === 'leave_item' &&
        action.item.hasNestedNavItems
      ) {
  ...
  auditAction(
    ...
    sourceAction: NavigationLeaveItemAction
  ) {
    ...
    return merge(
      ...
      map((actionOrMouseEvent) => {
        if (actionOrMouseEvent instanceof MouseEvent) {
          ...
          // Movement to the left will immediately trigger the change
          ...
          const clientRect =
            sourceAction.item &&
            sourceAction.item.nestedNav &&
            sourceAction.item.nestedNav.getBoundingClientRect();
          if (!clientRect) {
            return true;
          }
