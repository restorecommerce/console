...
import { ProductPrototypeFacade, RouterFacade } from '@console-core/state';
import { ICrudFeature, EUrlSegment } from '@console-core/types';
import { ModulesUiModule } from '@console-modules/ui';

@Component({
  selector: 'app-module-product-prototype-template',
  templateUrl: './prototype-template.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [AsyncPipe, ModulesUiModule],
})
export class PrototypeTemplateComponent implements OnInit, OnDestroy {
  ROUTER = ROUTER;
  featureRouter =
  ...
  feature: Readonly<ICrudFeature> = {
    name: {
      plural: 'Product Prototypes',
      singular: 'Product Prototype',
    },
    links: {
  ...
  readonly triggerRead$ = this.triggerRead
    .asObservable()
    .pipe(tap(() => this.productPrototypeFacade.read(this.queryVariables)));
  ...
  readonly triggerSearch$ = this.triggerSearch.asObservable().pipe(
    ...
    tap((value) => {
      ...
      this.productPrototypeFacade.read(this.queryVariables);
    })
  );
  ...
  readonly triggerSelectId$ = this.triggerSelectId
    .asObservable()
    .pipe(tap((id) => this.productPrototypeFacade.setSelectedId(id)));
  ...
  readonly triggerRemove$ = this.triggerRemove.asObservable().pipe(
    tap((id) => {
      ...
      this.productPrototypeFacade.remove({ id });
    })
  );
  ...
  readonly vm$ = combineLatest({
    dataList: this.productPrototypeFacade.all$,
    selectedProductPrototypeId: this.productPrototypeFacade.selectedId$,
    selectedProductPrototype: this.productPrototypeFacade.selected$,
